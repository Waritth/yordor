...
```mermaid
flowchart TD
    Home["หน้าแรก<br/>เลือกเกม (game picker)"] --> Pick{"เลือกเกม"}
    Pick -->|"Best 1 Best 2"| Setup
    Pick -->|"แมตช์/สโตรก/บ๊วยจ่ายหัว"| Soon["เร็วๆ นี้ (locked)"]
    Home -->|"เปิดจากลิงก์"| Play
...
หน้าแรก = **หน้าเลือกเกม** เกมแต่ละอันคือ game mode; v1 เปิดเฉพาะ **Best 1 Best 2** (Team mode) ที่เหลือ locked รอ roadmap
Stepper (หลังเลือกเกม): `ตั้งค่า → แต้มต่อ → เดิมพัน → เล่น → ผล`

---

## 2. หน้าแรก = เลือกเกม (Game Picker)
...
│                            │
│  ┌──────────────────────┐  │
│  │ ⛳ Best 1 Best 2      │  │  ← พร้อมเล่น (golf, mode=TEAM)
│  │ จับทีม Best1/Best2     │  │  → round.create(gameType=GOLF)
│  │ [ เล่น → ]            │  │
│  └──────────────────────┘  │
│  ┌──────────────────────┐  │
│  │ 🃏 ไพ่สามกอง          │  │  ← พร้อมเล่น (card, zero-sum)
│  │ นับแต้มรายตา รวม=0     │  │  → round.create(gameType=CARD3)
│  │ [ เล่น → ]            │  │
│  └──────────────────────┘  │
│  ┌──────────────────────┐  │
│  │ ⚔ แมตช์      🔒เร็วๆนี้│  │  ← locked
│  │ 🏌 สโตรก     🔒เร็วๆนี้│  │
│  │ 🪙 บ๊วยจ่ายหัว 🔒เร็วๆนี้│  │
│  └──────────────────────┘  │
│                            │
│  เปิดรอบจากลิงก์            │
│  [ วางลิงก์/โค้ดรอบ... ]    │  → /round/{token}
│                            │
│  รอบล่าสุดในเครื่องนี้       │  (token ใน localStorage)
│  • ก๊วนวันอาทิตย์  Best 1 Best 2 · 18 หลุม
└────────────────────────────┘
```

> หน้าแรกเป็น **เมนูเลือกเกม** — แต่ละการ์ด = เกมหนึ่ง (golf mode หรือ card game)
> เปิดเล่น: **Best 1 Best 2** (golf) + **ไพ่สามกอง** (card, zero-sum) · ที่เหลือ locked "เร็วๆ นี้"
> แตะ "เล่น" = `round.create({ gameType })` แล้วเข้า setup ของเกมนั้น (golf/card คนละ flow — ดู `CARD_3PILE_SPEC.md` §6)
> การ์ดที่ locked แตะแล้วขึ้น "กำลังพัฒนา" ไม่เข้า flow
...
|---|---|---|
| เลือกเกม (หน้าแรก) | localStorage รอบล่าสุด | — |
| เริ่มเกม Best 1 Best 2 | — | `round.create` (mode=TEAM) |
